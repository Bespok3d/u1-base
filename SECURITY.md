...
Bespok3d runs on stock printer firmware and never flashes custom firmware. This repo patches
Snapmaker's own Klipper source files directly, so the reports we care about most are:

- a patch here that changes what a stock, unregistered printer does (the base layer is meant to be a
  no-op on its own; a hook door that fires without a plugin registered against it is a bug we treat as
  a security issue, not a feature request),
- a hook door that lets a registered plugin bypass a print-time safety check it was not meant to reach
  (for example, forcing SET_PRINT_PREFERENCES mid-print in a way that corrupts the running job),
