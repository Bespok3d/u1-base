...
class PrintTaskConfig:
    def __init__(self, config):
        ...
        self._update_filament_flags_times = 10
        self._update_filament_flags_timer = self.reactor.register_timer(self._update_filament_flags_timer_handle)
    ...
    def _ready(self):
        ...
        self.reactor.update_timer(self._update_filament_flags_timer, self.reactor.NOW)

    def _handle_shutdown(self):
        if self._update_filament_flags_timer is not None:
            self.reactor.update_timer(self._update_filament_flags_timer, self.reactor.NEVER)
            self.reactor.unregister_timer(self._update_filament_flags_timer)
            self._update_filament_flags_timer = None

    def _update_filament_flags_timer_handle(self, eventtime):
        self.update_filament_flags()
        self._update_filament_flags_times -= 1
        if self._update_filament_flags_times > 0:
            return self.reactor.monotonic() + 0.3
